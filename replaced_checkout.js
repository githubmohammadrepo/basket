...
(function() {
    ...
    var hikashopCheckout = {
            ...
            form.addEventListener('submit', (event) => {
                ...
                window.jQuery.ajax(FetchPosturl, {
                        ...
                        response => {
                            ...
                            if ((htmlResponse.indexOf('ط¨ط§ طھط´ع©ط± ط§ط² ط®ط±ظٹط¯ ط´ظ…ط§')) && (htmlResponse.indexOf('You can now access your order'))) {
                                //user order was successful saved
                                window.location.href = "http://fishopping.ir/send-to-20"
                            } else {
        ...
        getBlockPos: function(el) {
            var ret = false;
            while (el && ret === false) {
                var pos = el.getAttribute("data-checkout-pos"),
                    step = el.getAttribute("data-checkout-step");
                if (pos && step)
        ...
        handleParams: function(data, url, req) {
            var t = this,
                fields = { type: 'blocktask', cid: 'cid', pos: 'blockpos', token: window.checkout.token };
            for (var f in fields) {
